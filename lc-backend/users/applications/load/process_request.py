...
class process_request:
    def get(user_id):
        try:
            response = supabase.table("applicants").select("*").eq("user_id", user_id).order("date_created", desc=True).execute()
            return JsonResponse({"message": "Applications retrieved successfully", "applications": response.data}, status=200)
        except Exception as e:
