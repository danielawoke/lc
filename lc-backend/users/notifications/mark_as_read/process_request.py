...
class process_request:
    def post(user_id, notification_id):
       try: 
           response = supabase.table("notifications").update({"read": True}).eq("user_id", user_id).eq("id", notification_id).execute()
           return JsonResponse({"message": "Notification marked as read successfully", "data": response.data}, status=200)
