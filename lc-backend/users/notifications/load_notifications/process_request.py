...
class process_request:
    def get(request, user_id):
         try:
              response = supabase.table("notifications").select("*").eq("user_id", user_id).order("time", desc=True).execute()

              return JsonResponse({"message": "Notifications loaded successfully", "data": response.data}, status=200)
         except Exception as e:
