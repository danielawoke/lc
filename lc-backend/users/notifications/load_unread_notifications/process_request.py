...
class process_request:
    def get(user_id):
       try: 
           response = supabase.table("notifications").select("*").eq("user_id", user_id).eq("read", False).order("time", desc=True).execute()
           return JsonResponse({"message": "Unread notifications loaded successfully", "data": response.data}, status=200)
