...
class process_request:
    def get(request, user_id):
        supabase_response = supabase.table("users").select("account_configured").eq("user_id", user_id).execute()
        return JsonResponse({"account_configured": supabase_response.data[0]["account_configured"]})
