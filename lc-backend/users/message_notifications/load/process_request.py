...
class process_request:
    def get(user_id):
        try:
            supabase_response = supabase.table("unread_messages").select("*").eq("user_id", user_id).order("created_at", desc=True).execute()
            return JsonResponse({"message": "Request processed successfully!", "data": supabase_response.data}, status=200)
