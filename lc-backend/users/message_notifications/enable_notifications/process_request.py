...
class process_request:
    def post(user_id):
        try:
            supabase.table("chat_room_relations").update({"notifications_enabled": True}).eq("user_id", user_id).execute()
            return JsonResponse({"message": "Notifications turned on for all chat rooms."}, status=200)
