...
class process_request:
    def post(request, user_id, friend_id):
        if(not request.data.get("private_key") or request.data.get("private_key") != settings.PRIVATE_KEY):
            return JsonResponse({"error": "Unauthorized"}, status=401)
        try:
            supabase_response = supabase.table("friends").delete().eq("user_id", user_id).eq("friend_id", friend_id).execute()
            try:
                supabase_response = supabase.table("chat_room_relations").update({"relation": "unknown"}).eq("user_id", user_id).eq("usertag", request.data.get("friend_usertag")).execute()
            except Exception as e:
                print(f"Error updating chat room relation: {str(e)}")
