...
class process_request:
    ...
    def post(request, user_id):
        try:
            ...
            supabase.table("users").update({"Premium": "Premium", "subscription_id": subscription_id}).eq("user_id", user_id).execute()
            return JsonResponse({"message": "Subscription resubscribed successfully."}, status=200)
