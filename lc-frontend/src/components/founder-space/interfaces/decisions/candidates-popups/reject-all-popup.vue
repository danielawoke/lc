...
async function rejectSumbit(){
    ...
    let context = document.getElementById('rejection-reason-text').value;

    closePopup();
    for (let i = props.job.candidates.length - 1; i >= 0; i--){
        await axios.post(`${apiURL}projects/votes/reject/${route.params.projectId}/${props.userID}`, {
            "reason": context,
            "application_id": props.job.candidates[i].application_id
        }).then((response) => {
            props.job.candidates.splice(i, 1);
        }).catch((error) => {
            console.log(error);
        });
    }
