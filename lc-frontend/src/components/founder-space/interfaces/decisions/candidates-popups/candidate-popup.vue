...
async function startDiscussionSelect(){
    
    internalTab.value = 1;
 
}

async function proposalSumbit(){
    // send proposal to backend
    // props.allCandidates.splice(props.allCandidates.findIndex(candidate => candidate.id === props.id), 1);
    console.log(props.candidate.application_object);
    let contextText = document.getElementById('context-text').value;
    console.log(props.jobID);
    await axios.post(`${apiUrl}projects/votes/create_proposal/${route.params.projectId}/${props.userID}/${props.jobID}/`, {
        "type": "Start Discussion",
        "context": contextText,
        "application_data": props.candidate.application_object,
        "application_id": props.candidate.application_id
    }).then((response) => {
        props.allCandidates.splice(props.allCandidates.findIndex(candidate => candidate.application_id === props.candidate.application_id), 1);
    }).catch((error) => {
        console.log(error);
    });
    await nextTick();
...
async function rejectSumbit(){
    
    let rejectionReason = document.getElementById('rejection-reason-text').value;
    
    await axios.post(`${apiUrl}projects/votes/reject/${route.params.projectId}/${props.userID}`, {
        ...
        <div id="popup" class="popup">
            ...
            <div v-if="internalTab === 0" class="container-popup">
                <div class="main-body-container">
                    <div style="position:absolute; margin-left:50px; width:98%; z-index: 40; height:calc( 100% - 200px); max-width: 1000px;">
                    ...
                    <!-- <div class="info-features" style="position:absolute; right: 25px; top:10px; width:100%; z-index: 40;">
                        <div class="star" style="width:40px; height:40px;" @click="startToggle($event.target)">
                            <img class="pin-icon" style="width:80%; height:80%; position: relative; top:10%; "  src="@/assets/founder-space/decisions/pin.png" :style="true ? 'background-image:linear-gradient(to bottom, #D13A00, #FFAA87); padding:15px; margin:-15px; border-radius:30%;': 'brightness(0.5)'"/>
                        </div>
                    </div> -->
                </div>
            </div>
            ...
            <div v-if="internalTab === 2" class="container-popup">
                <div class="main-body-container">
                    <div style="position:absolute; width:100%; height:calc( 100% - 200px); ">
                        <div style="position:relative; margin:auto; max-width:700px;">
                            <RejectionContextRequest :decision="candidate"/>
                        </div>
                    </div>
                </div>
            </div>
            <div v-if="internalTab === 0" class="popup-footer">
            ...
            <div v-if="internalTab === 2" class="popup-footer">
                <div @click="rejectSumbit" class="sumbit-rejection-button">
                    Sumbit Rejection
                </div>
            </div>
        </div>
