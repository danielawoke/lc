...
let tab = ref([1]);
let application_object = ref({application_object: {
    name: "",
    job: "",
    questions: []
}});
...
onMounted(() => {
    ...
    let peopleForTemp = [];
    let peopleAgainstTemp = [];

    for(let i = 0; i < props.decision.proposal["in favor"].length; i++){
        let person = props.decision.proposal["in favor"][i];
        peopleForTemp.push({
            name: person.name,
            image: person.image
        });
    ...
    for(let i = 0; i < props.decision.proposal["against"].length; i++){
        let person = props.decision.proposal["against"][i];
        peopleAgainstTemp.push({
            name: person.name,
            image: person.image
        });
    }

    peopleFor.value = peopleForTemp;
    peopleAgainst.value = peopleAgainstTemp;
    note.value = props.decision.proposal.note;
    application_object.value = props.decision.proposal;
...
async function confirmDelegate(){
    ...
    props.allDecisions.splice(props.allDecisions.findIndex(decision => decision.id === props.id), 1);
    closePopup();   
        ...
        <div id="popup" class="popup">
            ...
            <div v-if="!delegateSelect" class="container-popup">
                ...
                <div class="main-body-container">
                    <div style="position:absolute; left: 50px; width:100%; z-index: 40; height:calc( 100% - 100px); max-width: 600px;">
                        <CandidateApplication :decision="application_object"/>
                    </div>
                                ...
                                <div class="people-listing">
                                    ...
                                    <div v-for="(person, i) in peopleFor" :key="i">
                                        <div style="margin-bottom:10px; padding-bottom:10px; border-bottom: 1px solid rgb(200, 200, 200);">
                                            <img style="vertical-align: middle; margin-right:10px; width:40px; height:40px; border-radius:50%; display:inline-block;" :src="person.image" />
            ...
            <!-- <div v-if="!delegateSelect" class="popup-footer">
                <div @click="inFavorSelect" class="decision-button offer-button">
                    In Favor
                </div>
                <div @click="optionDelegate" class="decision-button delegate-button">
                    Delegate
                </div><div @click="withdrawSelect" class="decision-button withdraw-button">
                    Withdraw
                </div>
                <div @click="againstSelect" class="decision-button reject-button">
                    Against
                </div>
            </div> -->
            <!-- <div v-if="delegateSelect" class="popup-footer">
