...
async function startToggle(target){
    if(props.candidate.pinned){
        ...
        props.pinnedCandidates.splice(props.pinnedCandidates.findIndex(candidate => candidate.id === props.id), 1);
    } else {
        props.candidate.pinned = true;
        props.pinnedCandidates.unshift(props.candidateObject);
    }
    fontAdjust();
}
    ...
    <div @click.self="closePopup" id="popup-backdrop" class="popup-backdrop">
        <div id="popup" class="popup">
            ...
            <div class="popup-footer">
                <div class="decision-button offer-button">
                    Go to Discussion
                </div>
            </div>
        </div>
...
.offer-button{
    ...
    float:right;
    background-image: linear-gradient(to bottom, rgb(255, 169, 70), rgb(255, 236, 210));
}
.reject-button:hover{
