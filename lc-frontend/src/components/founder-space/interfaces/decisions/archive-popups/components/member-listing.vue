...
function selectDelegate(person){
    if(props.selectedId[0] == person.id){
        props.selectedId[0] = -1;
    } else {
        props.selectedId[0] = person.id;
    }
    console.log(props.selectedId[0]);
}
...
<template>
    <div class="application-box">
        <div v-if="members.length == 0" style="color: rgba(0, 0, 0,0.3); font-weight: lighter;">
            No Results
        </div>
        <div @click="selectDelegate(person)" :class="selectedId[0] == person.id ? 'person-block-selected' : 'person-block'" v-for="(person, i) in members" :key="i">
            <!-- {{person.id}} -->
            <div style="margin-bottom:20px; padding-bottom:20px; border-bottom: 1px solid rgb(200, 200, 200);">
                <img style="vertical-align: middle; margin-right:10px; width:80px; height:80px; border-radius:50%; display:inline-block;" :src="person.image" />
                <div class="people-listing-name" :style="selectedId[0] == person.id ? 'color: black;' : ''">{{person.name}}</div>
            </div>
        </div>
    </div>
</template>
