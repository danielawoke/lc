...
<template>
    <div class="application-box">
        <div v-if="decision.type == 'Start Discussion'" >
            <div style="font-size: 30px; margin-bottom: 20px; color:blue"><b>{{ decision.name }}</b></div>
            <div style="font-size: 23px; border-bottom: 1px solid #ccc; margin-bottom: 20px; color:black"><b>Position: </b><i>{{ decision.job }}</i></div>
            <div v-for="responses in decision.questions">
                <div style="margin-bottom: 10px;"><b>{{ responses.question }}</b></div>
                <div style="margin-bottom: 20px;">{{ responses.response }}</div>
            </div>
        </div>
        ...
        <div v-else-if="decision.type == 'Offer Membership'" >
            <!-- the link will go thier profile, something of the like, externally -->
            <div style="font-size:20px;"><b>Proposed Member: </b><i><a href="example">{{decision.name}}</a></i></div>
        </div>
        <div v-else-if="decision.type == 'Remove Member'" >
            <div style="font-size:20px;"><b>Proposed Member for Removal: </b><i><a href="example">{{decision.name}}</a></i></div>
        </div>
