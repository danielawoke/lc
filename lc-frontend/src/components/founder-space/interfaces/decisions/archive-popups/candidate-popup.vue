...
async function startToggle(target){

    //do something on the backend, remember that the pinned array is coming from a loaded version of the candidate's 
    //application from before, its not directly tied to some object staticly embdedded in the parent view.
    if(!props.candidate.pinned[0]){
        props.candidate.pinned[0] = true;
        target.classList.remove('pinned');
        props.allCandidates.splice(props.allCandidates.findIndex(candidate => candidate.id === props.id), 1);
        await nextTick();
        fontAdjust();
    } else {
        props.candidate.pinned[0] = false;
            ...
            <div v-if="!contextRequestPopup" class="container-popup">
                <div class="main-body-container">
                    ...
                    <div class="info-features" style="position:absolute; right: 25px; top:10px; width:100%; z-index: 40;">
                        <div :class="candidate.pinned ? 'star pinned' : 'star'" style="width:40px; height:40px;" @click="startToggle($event.target)">
                            <!-- <img class="pin-icon" style="width:80%; height:80%; position: relative; top:10%; "  src="@/assets/founder-space/decisions/pin.png" :style="candidate.pinned[0]? 'background-image:linear-gradient(to bottom, #D13A00, #FFAA87); padding:15px; margin:-15px; border-radius:30%;': 'brightness(0.5)'"/> -->
                        </div>
                    </div>
                </div>
            </div>
...
.offer-button{
    ...
    float:right;
    background-image: linear-gradient(to bottom, rgb(203, 70, 255), rgb(237, 210, 255));
}
.reject-button:hover{
