...
import MemberListing from '@/components/founder-space/interfaces/decisions/archive-popups/components/member-listing.vue';
...
onMounted(async () => {
    selectedId.value[0] = props.decision.delegate_id;
    console.log(selectedId.value[0]);
    await axios.get(`${apiURL}projects/votes/current_voters/${props.decision.proposal_id}`).then(response => {
        let data = response.data;
        members.value = data.proposals.map(voter => {
            console.log(voter);
            return {
                id: voter.user_id,
                name: voter.profile_data_cache.name,
                image: voter.profile_data_cache.profile_image
            }
        });
        let inFavor = data.proposals.filter(voter => voter.vote == "in favor");
        let against = data.proposals.filter(voter => voter.vote == "against");
        peopleFor.value = inFavor.map(voter => {
            return {
                name: voter.profile_data_cache.name,
                image: voter.profile_data_cache.profile_image
            }
        });
        peopleAgainst.value = against.map(voter => {
            return {
                name: voter.profile_data_cache.name,
                image: voter.profile_data_cache.profile_image
            }
        })
        filteredMembers = ref(members.value);
    });
    fontAdjust();
});
...
async function confirmDelegate(){
    ...
    try{
        await axios.post(`${apiURL}projects/votes/cast_vote/${props.userID}/${props.decision.proposal_id}`, {
            vote: "delegate",
            delegate_id: selectedId.value[0]
        });
    } catch(e){
        console.log(e);
        errorMessage.value = "Delegate cycle detected, please pick another member to delegate to";
        return;
    }
...
async function inFavorSelect(){
    ...
    props.decision.vote = "in favor";
    fontAdjust();

}

async function withdrawSelect(){
    try{
        await axios.post(`${apiURL}projects/votes/cast_vote/${props.userID}/${props.decision.proposal_id}`, {
            vote: "withdraw"
        });
...
<template>
    <div @click.self="closePopup" id="popup-backdrop" class="popup-backdrop">
        <div id="popup" class="popup">
            ...
            <div v-if="!delegateSelect" class="container-popup">
                <div class="decision-title">
                    {{ decision.type}}
                </div>
                <div class="main-body-container">
                    <div style="position:absolute; left: 50px; width:100%; z-index: 40; height:calc( 100% - 200px); max-width: 600px;">
                        <CandidateApplication :decision="decision"/>
                    </div>
                    <div class="info-features" style="position:absolute; right: 25px; top:10px; width:100%; z-index: 40;">
                        <div v-if="!noteMode" class="noteButton toggle" @click="activateNote"></div>
                        <div v-if="!inFavorView" class="infavor toggle" @click="activateInfavor"></div>
                        <div v-if="!againstView"  class="against toggle" @click="activateAgainst"></div>
                        <div id="note-block" v-if="noteMode || inFavorView || againstView">
                            <div v-if="noteMode" class="note">
                                <div class="note-header">
                                    <div class="note-header-label">
                                        Note:
                                    </div>
                                    <div class="minimize noteButton" @click="closeNote">
                                        -
                                    </div>
                                </div>
                                ...
                                <div class="note-footer">Alex smith</div>    
                            </div>
                            <div v-else-if="inFavorView" class="note">
                                <div class="note-header">
                                    <div class="note-header-label">
                                        In favor
                                    </div>
                                    <div class="minimize infavor" @click="closeFavor">
                                        -
                                    </div>
                                </div>
                                <div class="people-listing">
                                    <div v-if="peopleFor.length == 0" style="color: rgba(0, 0, 0,0.3); font-style: italic; transform: scaleY(1.2); font-weight: lighter;">
                                        Seems a litte unpopular here...
                                    </div>
                                    <div v-for="(person, i) in peopleFor" :key="i">
                                        <div style="margin-bottom:10px; padding-bottom:10px; border-bottom: 1px solid rgb(200, 200, 200);">
                                            <img style="vertical-align: middle; margin-right:10px; width:40px; height:40px; border-radius:50%; display:inline-block;filter: invert(1);" :src="person.image" />
                                            <div class="people-listing-name">{{person.name}}</div>
                                        </div>
                                    </div>
                                    <div v-if="peopleFor.length > 0" style="color: rgba(0, 0, 0,0.3); font-style: italic; transform: scaleY(1.2); font-weight: lighter;">
                                        {{ peopleFor.length }} {{ peopleFor.length == 1 ? 'person is' : 'people are' }} in favor of this.
                                    </div>
                                </div>  
                                <div style="color:#00000000" class="note-footer">
                                    .
                                    <!-- {{ peopleFor.length }} people in favor -->
                                </div>    
                            </div>
                            <div v-else-if="againstView" class="note">
                                <div class="note-header">
                                    <div class="note-header-label">
                                        Against
                                    </div>
                                    <div class="minimize against" @click="closeAgainst">
                                        -
                                    </div>
                                </div>
                                <div class="people-listing">
                                    <div v-if="peopleAgainst.length == 0" style="color: rgba(0, 0, 0,0.3); font-style: italic; transform: scaleY(1.2); font-weight: lighter;">
                                        No complaints!
                                    </div>
                                    <div v-for="(person, i) in peopleAgainst" :key="i">
                                        <div style="margin-bottom:10px; padding-bottom:10px; border-bottom: 1px solid rgb(200, 200, 200);">
                                            <img style="vertical-align: middle; margin-right:10px; width:40px; height:40px; border-radius:50%; display:inline-block; filter: invert(1);" :src="person.image" />
                                            <div class="people-listing-name">{{person.name}}</div>
                                        </div>
                                    ...
                                    <div v-if="peopleAgainst.length > 0" style="color: rgba(0, 0, 0,0.3); font-style: italic; transform: scaleY(1.2); font-weight: lighter; ">
                                        {{ peopleAgainst.length }} {{ peopleAgainst.length == 1 ? 'person is' : 'people are' }} against this.
                                    </div>
                                </div>  
                                <div style="color:#00000000" class="note-footer">
                                    .
                                    <!-- {{ peopleAgainst.length }} people against -->
                                </div>    
                            </div>
                        </div>
                    </div>
            ...
            <div v-if="delegateSelect" class="container-popup">
                ...
                <div class="main-body-container">
                    ...
                    <div style="color:cyan; font-size: 13px; font-family:Arial, Helvetica, sans-serif; position: absolute; bottom: -110px; left: 10px;">{{ errorMessage }}</div>
                </div>
            </div>
            <div v-if="!delegateSelect" class="popup-footer">
                <div @click="inFavorSelect" :class="decision.vote=='in favor' ? 'decision-button-selected' : 'decision-button'">
                    In Favor
                </div>
                <div @click="optionDelegate" :class="decision.vote=='delegate' ? 'decision-button-selected' : 'decision-button'">
                    Delegate
                </div><div @click="withdrawSelect" :class="decision.vote=='withdraw' ? 'decision-button-selected' : 'decision-button'">
                    Withdraw
                </div>
                <div @click="againstSelect" :class="decision.vote=='against' ? 'decision-button-selected' : 'decision-button'">
                    Against
                </div>
            </div>
            <div v-if="delegateSelect" class="popup-footer">
...
.decision-button-selected{
    ...
    background-image: linear-gradient(to bottom, rgb(0, 0, 0), rgb(146, 146, 146));
    border: solid rgb(243, 255, 214) 2px;
...
.decision-button-selected:hover{
    border: solid rgb(255, 255, 255) 2px;
    background-image: linear-gradient(to bottom, rgb(0, 0, 0), rgb(105, 105, 105));
    cursor: pointer;
}
.decision-button{
    float: right;
    width:calc(25% - 4px);
    height:calc( 100% + 20px);
    background-image: linear-gradient(to bottom, rgb(224, 224, 224), rgb(255, 255, 255));
...
.decision-button:hover{
    background-image: linear-gradient(to top, rgb(255, 255, 255), rgb(167, 167, 167));
    cursor: pointer;
}
