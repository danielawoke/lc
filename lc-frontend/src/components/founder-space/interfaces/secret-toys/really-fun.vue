...
onMounted (async () => {
    ...
    document.getElementsByClassName("BW-wheel-container")[0].addEventListener("mousedown", (event) => {
        let selector = document.getElementsByClassName("color-wheel-selector")[0];
        let x = event.offsetX / event.target.offsetWidth;
        let y = event.offsetY / event.target.offsetHeight;
        x = xAdjust(x,y);
        ...
        let circularY = elipseCenteredAltY(circularX,y*100);
        selector.style.top = (circularY) + "%";
        document.getElementsByClassName("BW-wheel-selector")[0].style.top = (circularY) + "%";
        document.getElementsByClassName("color-wheel-selector")[0].style.left = (circularX) + "%";

        document.getElementsByClassName("BW-wheel-container")[0].addEventListener("mousemove", BWWheelEventListener);
    });
    document.getElementsByClassName("BW-wheel-container")[0].addEventListener("mouseup", (event) => {
        document.getElementsByClassName("BW-wheel-container")[0].removeEventListener("mousemove", BWWheelEventListener);
    });
