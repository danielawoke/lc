...
onMounted(async () => {
    ...
    try{
        ...
        await axios.get(`${import.meta.env.VITE_APP_API_URL}jobs/project-load/${route.params.projectId}/true`)
            .then(response => {
                let data = response.data;
        ...
        if(route.query.job_id){
            console.log("Job ID from query:", route.query.job_id);  
            preLoadPayload.value.job_id = route.query.job_id;
            preLoadPayload.value.jobInformation = projectProfileData.value.jobsCard.jobs.find(job => job.job_id === route.query.job_id).job_object;
            preLoadPayload.value.render[0] = true;
        }
...
-->
<Header />
<div class="container" :style="{width: '100vw', overflowX:'hidden'}">
    <MainCard :mainCardData="projectProfileData.mainCard" :viewMode="true" />
    <RolesCard :jobsCardData="projectProfileData.jobsCard" :viewMode="true" :projectTitle="projectProfileData.mainCard.title" :preLoadPayload="preLoadPayload"/>
    <ExtraCard v-for="(card, index) in projectProfileData.extraCards" :key="card.id" :cardInfo="card" :extraCards="projectProfileData.extraCards" :viewMode="true"/>
</div>
